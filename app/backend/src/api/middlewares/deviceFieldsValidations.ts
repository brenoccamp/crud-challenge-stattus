...
export const versionFieldValidate = (
  ...
  try {
    ...
    if (!version) return res.status(BAD_REQUEST_STATUS_HTTP).json(ERROR_MISSING_FIELD);
...
export const tagsFieldValidate = (
  ...
  try {
    ...
    if (!tags) return res.status(BAD_REQUEST_STATUS_HTTP).json(ERROR_MISSING_FIELD);
